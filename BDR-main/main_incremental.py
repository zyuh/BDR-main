    ...
    for t, (_, ncla) in enumerate(taskcla):
        ...
        if t > args.resume_task:
            ...
            if args.ddp:
                ...
                if args.approach == 'lucir_cwd_la_my4':
                    net.add_head_v2(taskcla[t][1], use_norm=True, use_noise=False)
                    net.to(device)
                elif args.approach == 'lucir_cwd_la_my8':
                    num_classes = 100
                    ...
                    init_weight = torch.nn.Parameter(torch.mm(weight, torch.eye(num_classes, 512)))

                    net.add_head_v3(taskcla[t][1], init_weight, t)
                    net.to(device)
                    pass
                else:
