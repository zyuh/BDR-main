...
class MaskDecorrelateLossClass(nn.Module):

    def __init__(self, reject_threshold=1, ddp=False):
        super(MaskDecorrelateLossClass, self).__init__()
        self.eps = 1e-8
        self.reject_threshold = reject_threshold
        self.ddp = ddp

    def forward(self, x, y, t, taskcla):
        assert t > 0
        _, C = x.shape
        if self.ddp:
            # if DDP
        ...
        old_classes = 0
        for each_t in range(len(taskcla)):
            if each_t < t:
                old_classes = old_classes + taskcla[each_t][1]
            else:
                current_classes_num = taskcla[each_t][1]
                break
